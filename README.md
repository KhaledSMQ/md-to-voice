...
- **Word-by-word highlighting** synced to the audio, layered on top of the rendered Markdown — Unicode-aware word segmentation via `Intl.Segmenter`.
...
2. A custom remark plugin walks the mdast and replaces every text node with a sequence of `word` and `whitespace` nodes, each `word` carrying a globally-incrementing index. Word splitting uses `Intl.Segmenter` (`granularity: 'word'`).
...
7. Inside each chunk, a `requestAnimationFrame` loop maps `audio.currentTime` to the active word using a character-length-weighted timing curve (a karaoke-style lookahead trick).
...
| Tokenization | `Intl.Segmenter`, `unist-util-visit` |
