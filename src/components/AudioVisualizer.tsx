...
function idleBreath(): number {
  return 0.1 + Math.sin(Date.now() / 850) * 0.035
}
...
export function AudioVisualizer({ analyserRef, playerStatus, className = '' }: Props) {
  ...
  }, [analyserRef, isPlaying, isPaused, isLoading])
  ...
  return (
    ...
    >
      <div
        ref={containerRef}
        className={`h-32 w-full transition-opacity duration-500 ${isLive ? 'opacity-100' : 'opacity-90'}`}
        aria-hidden
      />
